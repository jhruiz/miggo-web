...
function pagarCuenta(cuentapendienteId) {
    $("#div_pagarcuenta").load(
        $('#url-proyecto').val() + "cuentasclientes/obtenercuentacliente", { pagoId: cuentapendienteId },
        function() {
...
function eliminarCuenta(id, valPendiente) {

    if (valPendiente > 0) {
        alert('No es posible eliminar la cuenta ya que tiene un saldo pendiente');
    } else {
